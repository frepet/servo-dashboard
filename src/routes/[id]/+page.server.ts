...
const backwardsCompatability = (state: State) => {
  ...
  if (state.version == 19) state = v19tov20(state);
  return state;
};
...
const v17tov18 = (state: State) => {
  console.log('v17 -> v18');
  state.version = 18;
  state.mecanumsteers = state.mecanumsteers.map((old) => {
    return {
      ...old,
      frontLeftMotorId: -1,
      frontRightMotorId: -1,
...
const v19tov20 = (state: State) => {
  console.log('v19 -> v20');
  state.version = 20;
  state.motors = state.motors.map((old) => {
    return {
      ...old,
      axis2: -1,
    }
