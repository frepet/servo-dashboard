...
export function drawTopDownArm(context: CanvasRenderingContext2D, ik: IK) {
	...
	drawCircle(
		context,
		hand,
		Math.min(Math.max(6 * ((ik.arm[1].y + ik.base.y) / 100), 4), 12),
		'#000099'
	);
	...
	drawCircle(
		context,
		elbow,
		Math.min(Math.max(6 * ((ik.arm[0].y + ik.base.y) / 100), 4), 12),
		'#009900'
	);
...
export function drawRightSideArena(
	context: CanvasRenderingContext2D,
	base: Vec2,
	boundingBox: Vec2[],
	armLength: number
...
export function drawRightSideArm(
	context: CanvasRenderingContext2D,
	base: Vec2,
	arm: Vec2[],
	target: Vec2
