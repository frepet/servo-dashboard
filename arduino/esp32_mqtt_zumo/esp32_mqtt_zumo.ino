...
const char *BROKER_URL = "mqtt://";
const char *BROKER_USERNAME = "a";
const char *BROKER_PASSWORD = "a";
const uint8_t KEEP_ALIVE_SECONDS = 5;

// START of Robot Settings
static const String TOPIC_PREFIX = "robot";
const int MAX_SERVOS = 0;
const int MAX_MOTORS = 2;
Motor motors[MAX_MOTORS] = {Motor(0), Motor(1)};
...
void updateServos(const String &topic, const String &payload) {
  if (topic.startsWith(SERVOS_PREFIX)) {
    ...
    if (servoNumber >= 0 && servoNumber < MAX_SERVOS) {
      servos[servoNumber].update(map(payload.toInt(), 0, 255, -100, 100));
    } else {
